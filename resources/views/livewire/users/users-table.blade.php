<div class="space-y-4">
                            ...
                            <div wire:ignore>
                                <select style="height: 30px;" id="roles-select" multiple>
                                    @foreach (\Spatie\Permission\Models\Role::all() as $role)
                                        <option value="{{ $role->name }}" @if(array_key_exists($role->name, $selectedRoles ?? [])) selected @endif>{{ $role->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </x-th-table-search>
    ...
    {{-- @livewire('users.user-roles-modal') --}}
        ...
        <script>
            ...
            function initSelect2() {
                ...
                $('#roles-select').on('change', function (e) {
                    ...
                    @this.set('searchTerm.selectMultiple.roles', data);
                });
            };
        </script>
    @endpush
