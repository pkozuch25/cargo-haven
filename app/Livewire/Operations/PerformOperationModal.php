...
class PerformOperationModal extends ModalComponent
{
    ...
    public function performOperationModal(DispositionUnit $operation) : void
    {
        ...
        $this->operation->loadMissing('disposition');
        $this->disposition = $operation->disposition;
    ...
    {
        $this->validate();
        $transshipmentCardService = new TransshipmentCardService();
        ...
        if ($this->relationToCarriage()) {
            $transshipmentCardService->calcGrossWeight($cardUnit);
        }

        $card = null;

        if ($this->selectedCard == 'newCard') {
            $card = $transshipmentCardService->createNewCard($this->operation);
        } else {
    ...
    private function performOperationToCarriage(TransshipmentCardUnit $cardUnit) : void
    {
        if ($this->relationToYard()) {
    ...
    private function performOperationToTruck(TransshipmentCardUnit $cardUnit) : void
    {
        if ($this->relationToYard()) {
            $cardUnit->tcu_net_weight = $this->netWeight;
